...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  console.log(
    '💡 Make sure to copy .env.example to .env.local and fill in your values',
  );
  process.exit(1);
}
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes('relation "allowlist" does not exist')) {
        console.log(
          '💡 Database table "allowlist" not found. Expected for new setup.',
        );
        console.log(
          '📝 This table should be created in previous foundation tasks.',
        );
        return false;
      } else if (error.message.includes('Invalid API key')) {
        console.log(
          '💡 Invalid API key. Check your EXPO_PUBLIC_SUPABASE_ANON_KEY',
        );
        return false;
    ...
    try {
      ...
      if (error) {
        if (error.message.includes(`relation "${table}" does not exist`)) {
          console.log(
            `⚠️  Table "${table}" not found (expected for new setup)`,
          );
...
async function runTests() {
  ...
  const accessibleTables = results.schema.filter(
    (r) => r.status === 'accessible',
  ).length;
  const totalTables = results.schema.length;
  console.log(
    `- Database Schema: ${accessibleTables}/${totalTables} tables accessible`,
  );
...
export { supabase, testConnection, testAuth, testDatabaseSchema, runTests };
